...
instance [∀ α, MvFunctor <| F α] : MvFunctor (Sigma F) where map := fun f ⟨a, x⟩ => ⟨a, f <$$> x⟩


section
  variable [q : ∀ α, MvQPF <| F α]
...
instance [p : ∀ α, IsPolynomial <| F α] : IsPolynomial (Sigma F) where
  repr_abs := by
    intros α x
    rcases x with ⟨a, f⟩
    simp only [repr, abs, Sigma.repr, Sigma.abs]
